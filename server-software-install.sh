...
if [ ! -x /usr/local/bin/restic ]; then
  set -x
  TMP_DIR=$(mktemp -d 2>/dev/null || mktemp -d -t 'restic-work')
  pushd "$TMP_DIR"
  if uname -a | grep -c -i arm >/dev/null; then
    # NOTE(cdzombak): arm64 is also available, but I don't have any Linux/arm64 systems ATM.
    #                 When I do this install process will need to be updated.
