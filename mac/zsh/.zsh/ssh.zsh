...
alias hosts-list-md='{ grep "^#\?Host " ~/.ssh/config | grep -v -- "kvm.dzhome$" | grep -v "bitbucket" | grep -v "Host gh" | grep -v "github" | grep -v "Host \*" | sed "s/^Host //" | sed "s/^#Host /!/" | sed "s/ [^ ]*\.tailnet-003a.ts.net//g" | sed "s/ [^ ]*\.dzhome//g" | sed "s/^/- [ ] /" ; }'
