...
alias hosts-list="{ grep \"^#\?Host \" ~/.ssh/config | grep -v -- \"kvm.dzhome$\" | grep -v \"bitbucket\" | grep -v \"Host gh\" | grep -v \"github\" | grep -v \"Host \*\" | sed \"s/^Host //\" | sed \"s/^#Host /!/\" | awk '{print \$1}' ; }"
alias hosts-list-md="{ grep \"^#\?Host \" ~/.ssh/config | grep -v -- \"kvm.dzhome$\" | grep -v \"bitbucket\" | grep -v \"Host gh\" | grep -v \"github\" | grep -v \"Host \*\" | sed \"s/^Host //\" | sed \"s/^#Host /!/\" | awk '{print \$1}'  | sed 's/^/- [ ] /' ; }"
